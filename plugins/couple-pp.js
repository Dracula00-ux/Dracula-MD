...
const couplepp = async (m, Matrix) => {
  ...
  await m.React('💑');
  await Matrix.sendMessage(m.from, { text: "*💑 Retrieving couple profile pictures...*" }, { quoted: m });

  try {
    const response = await axios.get("https://api.davidcyriltech.my.id/couplepp");

    if (!response.data || !response.data.success) {
      return Matrix.sendMessage(m.from, { text: "❌ Failed to retrieve couple profile pictures. Please try again later." }, { quoted: m });
    }
    ...
    console.error(error);
    Matrix.sendMessage(m.from, { text: "❌ An error occurred while retrieving the couple profile pictures." }, { quoted: m });
  }
};
