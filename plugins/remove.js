  ...
  try {
    ...
    await gss.groupParticipantsUpdate(m.from, validUsers, 'remove')
      .then(() => {
        ...
        m.reply(`*The users ${kickedNames} have been successfully removed from the group ${groupMetadata.subject}.*`);
      })
      .catch(() => m.reply("⚠️ Failed to remove the user(s) from the group."));
  } catch (error) {
